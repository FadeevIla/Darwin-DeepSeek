...
class LLMInterface:
    ...
    def generate_feature(self, code):
        ...
        base_prompt = (
            ...
            "- incubate_cmd — высиживание яйца\n"
            "- feed_cmd — кормление\n"
            "- train_cmd — тренировка\n"
            "- battle_cmd — битва с ботом или игроком\n"
            "- top_cmd — таблица лидеров\n"
            "- stats_cmd — статистика питомца\n"
            "- help_cmd — справка\n"
            "- report_cmd — фидбек от админа\n\n"
            "ЧТО ДЕЛАТЬ (выбери ОДНО):\n"
            "1. УЛУЧШИТЬ существующую функцию — добавь docstring, константы, валидацию, логирование\n"
            "2. СОЗДАТЬ новую функцию — если нужна отсутствующая команда (/shop, /inventory, /buy, /use, /duel)\n\n"
            "ДЛЯ НОВЫХ ФУНКЦИЙ:\n"
            "- Полноценная реализация, не заглушка\n"
            "- docstring, константы, валидация, логирование — как у существующих\n"
            "- Интеграция с get_player()/update_player()\n\n"
            "КРИТИЧЕСКИ ВАЖНО:\n"
            "- Верни ТОЛЬКО одну функцию ЦЕЛИКОМ (новую или улучшенную)\n"
            "- Если создаёшь новую — она будет автоматически добавлена в bot.py\n"
